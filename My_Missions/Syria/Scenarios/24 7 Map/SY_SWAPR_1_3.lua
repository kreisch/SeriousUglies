--- === SWAPR CONFIG ===
  ...
  local SWAPR_Prefixes = { "_Client" , "_Helo" , "_Plane" , "_Test" }  -- Add all the relevant client prefixes / suffixes to this table
...
local function Spawner()
   ...
   --MESSAGE:New("Spawner called!", 10):ToAll()
   ...
   for i, GroupTable in pairs( Reference_Unit_Table ) do
       ...
       if ClientSET:FindClient(GroupTable.ClientName) == nil and GroupTable.ReplacementType == "AI" then
          
           -- MESSAGE:New("ClientSET:FindClient(GroupTable.ClientName) == nil and GroupTable.ReplacementType == AI", 10):ToAll()
       ...
       --MESSAGE:New("Spawner runs until second check", 10):ToAll()
       
       
       if ClientSET:FindClient(GroupTable.ClientName) == nil and GroupTable.ReplacementType == "Static" and GroupTable.Sheltered == "false" and Destroyed_Replacement_Table[GroupTable.ClientName.."_Replacement"] == "Enabled" and ClientReplaced_Table[GroupTable.ClientName.."_Replacement"] ~= nil then
          
          if StaticObject.getByName(GroupTable.ClientName.."_Replacement") == nil or StaticObject.getByName(GroupTable.ClientName.."_Replacement"):isExist() == false then
   ...
   --MESSAGE:New("Spawner runs until the end", 10):ToAll()
end
  

local function DestroyHeading(Initiator)
      Initiator:destroy()
end
...
function Birth_Event_Handler_MOOSE:OnEventBirth(EventData)   
   ...
   --MESSAGE:New("Birth event detected!", 10):ToAll()
   
   if EventData.initiator:getCategory() == Object.Category.UNIT and Replacement_Type == "Static" and Heading_Table[EventData.initiator:getName()] ~= nil then
          
      Heading_Table[EventData.initiator:getName()].units[1].heading = UTILS.ToRadian( UNIT:Find(EventData.initiator):GetHeading() )
      
      -- MESSAGE:New(EventData.initiator:getName().."marked for removal!", 10):ToAll()
   ...
   local ClientSET2 = SET_CLIENT:New():FilterActive(Active):FilterPrefixes(SWAPR_Prefixes):FilterOnce()
       ...
       if ClientSET2:FindClient(EventData.initiator:getName()) and Reference_Unit_Table[EventData.initiator:getName().."_Replacement"] ~= nil then
          ...
          if Reference_Unit_Table[EventData.initiator:getName().."_Replacement"].CarrierName == 0 then   
             ...
             Spawner()                                                
              --MESSAGE:New(EventData.IniPlayerName.." has taken control of "..EventData.IniUnitName, 10):ToAll()    
          end       
       end
       ...
       if EventData.initiator:getCategory() == Object.Category.UNIT then
          ...
          if EventData.initiator:getPlayerName() == nil and Reference_Unit_Table[EventData.initiator:getName()] ~= nil then 
             ...
             local Spawned_Replacement = { 
                ...
                --MESSAGE:New(EventData.initiator:getName().." born and added to Spawned_Replacement_Table!\nCategory = "..EventData.initiator:getCategory(), 10):ToAll()
          end
   ...
   --MESSAGE:New("Birth event runs until the end!", 10):ToAll()
end


local function AliveCheck(Replacement_Param_Table)
   
      if Replacement_Param_Table.ReplacementHit:isExist() == false then   
         ...
         if Spawned_Replacement_Table[Replacement_Param_Table.ReplacementClientName.."_Replacement"] ~= nil then
            ...
            --local FlagValue = trigger.misc.getUserFlag(Replacement_Param_Table.ReplacementClientName.."_Destroyed")
             
            --MESSAGE:New(Replacement_Param_Table.ReplacementClientName.."_Replacement doesn't exist!\nFlag "..Replacement_Param_Table.ReplacementClientName.."_Destroyed"..
            --" set to "..FlagValue.."\nCleared from Spawned_Replacement_Table!", 10):ToAll()
         end
      end 
end
   ...
   function Hit_Event_Handler_MOOSE:OnEventHit(EventData)
      ...
      for i , ReplacementParam in pairs( Spawned_Replacement_Table ) do
          ...
          if Target == ReplacementParam.Initiator then
             ...
             timer.scheduleFunction(AliveCheck, Replacement_Param_Table , timer.getTime() + 3)
          end
      end
   end 
end
...
local function SpawnHeadingStatics(Param_Table)
       ...
       --MESSAGE:New("SpawnHeadingStatics called", 10):ToAll()
      ...
      local Replacement_Static_1 = {  
                                    ...
                                    --MESSAGE:New(Param_Table.Client_Name.."_Replacement" , 10):ToAll()
...
local function StaticHeading_Madness(Database_Template_Table , ClientName , Airbase_Area , Game_Mode) 
                             ...
                             for k , unit_subtable in pairs( Database_Template_Table.units ) do 
                                 ...
                                 if Database_Template_Table.units[k].name == ClientName then
                                    ...
                                    if type(k) == "number" then
                                       ...
                                       Heading_Template.units[1].unitId = nil
  ...
  for i , Unborn_Client in pairs( SWAPR_SET:GetSetObjects() ) do
      ...
      if Unborn_Client:GetName() then
         ...
         if _DATABASE:GetGroupTemplateFromUnitName(Unborn_ClientName) ~= nil then
            ...
            local ClosestAirbase, Airbase_Distance = ReferenceCoord:GetClosestAirbase(Airbase.Category.AIRDROME)
                
            local ClosestFARP, FARP_Distance = ReferenceCoord:GetClosestAirbase(Airbase.Category.HELIPAD)
            
            local ClosestCarrier, Carrier_Distance = ReferenceCoord:GetClosestAirbase(Airbase.Category.SHIP)
            
            local ParkCoord = ReferenceCoord:GetClosestParkingSpot(ClosestAirbase)
            
            
            if ReferenceCoord:Get2DDistance(ParkCoord) < 20 and ClosestAirbase ~= nil then
            ...
            elseif ReferenceCoord:Get2DDistance(ParkCoord) >= 20 and ClosestAirbase ~= nil then
                  
                   Sheltered = false
                   Park_Spot = false
                   
                   if Airbase_Distance < 6000 then 
                     
                      Airbase_Area = true
                      
                   elseif Airbase_Distance >= 6000 then
                ...
                if Carrier_Distance == nil or Carrier_Distance >= 300 then
                   if FARP_Distance == nil or FARP_Distance >= 100 then
                      ...
                      or ( Sheltered == false and Replacement_Type == "Static" and Airbase_Area == false ) then
                         ...
                         StaticHeading_Madness(Database_Template_Table , Unborn_ClientName , Airbase_Area , Game_Mode )
                      end           
                   end           
                end            
                ...
                if Carrier_Distance == nil or Carrier_Distance >= 300 then
                   if FARP_Distance == nil or FARP_Distance >= 100 then
                      ...
                      or ( Replacement_Type == "Static" and Sheltered == false and Airbase_Area == true and Database_Template_Table.route.points[1].type == "TakeOffGround" and Database_Template_Table.route.points[1].action == "From Ground Area" ) then
                            ...
                            local Unit_Template_1 = {
                             ...
                             if Unit_Template_1.route.points[1].type == "TakeOffGround" and Unit_Template_1.route.points[1].action == "From Ground Area" and Airbase_Area == true then
                                
                                Unit_Template_1.route.points[1].type = "TakeOffGroundHot"
                                Unit_Template_1.route.points[1].action = "From Ground Area Hot"
                             end
                ...
                if FARP_Distance ~= nil and FARP_Distance < 100 then
...
function FlagChecker()
        ...
        --MESSAGE:New("FlagChecker running!", 10):ToAll()
         
    for i , GroupTable in pairs( Reference_Unit_Table ) do
       
        if trigger.misc.getUserFlag(GroupTable.ClientName) ~= nil and trigger.misc.getUserFlag(GroupTable.ClientName) ~= 0 then     
           
           local Flag_Check = trigger.misc.getUserFlag(GroupTable.ClientName)
           
           if Unit.getByName(GroupTable.ClientName.."_Replacement") ~= nil then
              
              --MESSAGE:New("Flag_Check = "..Flag_Check.."\n"..GroupTable.ClientName.."_Replacement".." found and marked for removal!", 10):ToAll()
  ...
  if Game_Mode == "SP" then
     ...
     function Handler:onEvent(event)
              ...
              if ( event.id == world.event.S_EVENT_PLAYER_ENTER_UNIT ) and event.initiator:getPlayerName() ~= nil then 
                ...
                -- MESSAGE:New(event.initiator:getName().." entered unit!", 10):ToAll()       
                 
                 if Unit.getByName(event.initiator:getName().."_Replacement") ~= nil then
             
                   -- MESSAGE:New(event.initiator:getName().."_Replacement detected and marked for removal!", 10):ToAll() 
         ...
         --MESSAGE:New("S_EVENT_PLAYER_ENTER_UNIT runs until the end!", 10):ToAll()   
  ...
  "\nSheltered_Replacements = "..tostring(Sheltered_Replacements), 20):ToAll()
  
  MESSAGE:New("SWAPR_SET count = "..SWAPR_SET:Count(), 10):ToAll()