...
-- Changes by kreisch START
-- CASEVAC

local TICcasevac = false
---- Changes by kreisch END
...
function GoldTIC(_args,TICGridZone)
  ...
  Friendly:OnSpawnGroup(function (spawngroup)
      ...
      -- Edited by kreisch for CASEVAC START
        if TICcasevac then
          -- Get Zone of cannonfoddercoord1 unit
          -- Spawn CSAR at this zone
        local identifier = math.random(1, 499)
        cannonfodder1Zone = ZONE_UNIT:New( "Zone Casevac" .. identifier, unit1, 100 )
        my_csar:SpawnCSARAtZone( cannonfodder1Zone:GetName(), coalition.side.BLUE, "Unlucky Ugly", true )

        end

      -- Edited by kreisch for CASEVAC END
  end)
  Friendly:Spawn()
end --function
...
local function TICcasevacOn()
  TICcasevac = true
end

local function TICcasevacOff()
  TICcasevac = false
end
...
if showTICmenu == true then
  ...
  TROOPSINCONTACTcasevacOn = MENU_COALITION_COMMAND:New( coalition.side.BLUE,"TIC CASEVAC ON",MenuCoalitionTopLevel,TICcasevacOn, "")
  TROOPSINCONTACTcasevacOff = MENU_COALITION_COMMAND:New( coalition.side.BLUE,"TIC CASEVAC OFF",MenuCoalitionTopLevel,TICcasevacOff, "")
end
