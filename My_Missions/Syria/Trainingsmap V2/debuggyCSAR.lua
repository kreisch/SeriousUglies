...
local debugGroupSet=SET_GROUP:New():FilterCoalitions("blue"):FilterCategoryGround():FilterZones({ZONE:New("MT01Area")}):FilterStart() 
trigger.action.outText("debugy." .. debugGroupSet:Count() , 30)
...
debugGroupSet:HandleEvent( EVENTS.Dead )
    function debugGroupSet:OnEventDead( EventData)
            local _unit = EventData.IniUnit
            local _name = _unit:GetName()
            local _group = _unit:GetGroup()
            trigger.action.outText("Unit died.." .. _name .. " Part of group " .. _group:GetName(), 30)
