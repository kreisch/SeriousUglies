...
my_ctld = CTLD:New(coalition.side.BLUE,{"helicargo", "UH1H", "UH60", "SA342","MI8","Mi24","MI24","Mi-24","MI-24","Mi","MI","mi","KA50","AH64D", "Hercules", "C130"},"CTLD_Blue")
...
function my_ctld:OnAfterCratesBuild(From,Event,To,Group,Unit,Vehicle)
  ...
  if (string.match(vunitname, "FOB")) then --Get Unit Type : Wenn FOBCrate dann los
    my_ctld:AddCTLDZone(vname,CTLD.CargoZoneType.LOAD,nil,true,true)
  ...
  elseif string.match(vunitname, "FARP") then
    ...
    -- local zone = ZONE_GROUP:New("FARP Zone" .. farpCounter, vgroup, 10)
      local zone = ZONE_GROUP:New("FARP Zone", vgroup, 60)
