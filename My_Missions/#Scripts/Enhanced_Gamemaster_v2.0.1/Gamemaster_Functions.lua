--Gamemaster_Functions v. 2.0.1, created by Cake - TaktLwG 66
...
local DefaultCountry = nil --All units spawned via the "-s" command will be spawned as units of the country specified here. For countrynames see documentation. Does NOT override the country set by the parameter of the "-s" command.
...
local function Smoke(param1, mcoord)
  ...
  if param1 == "b" then color = "Blue"
  elseif param1 == "g" then color = "Green"
  elseif param1 == "o" then color = "Orange"
  elseif param1 == "r" then color = "Red"
  elseif param1 == "w" then color = "White"
  end
  
  mcoord:Smoke(SMOKECOLOR[color])
  
  if DebugMode == true
  
    then trigger.action.outText("DEBUG: ".. color .."-Smoke aktiv!", 10)
...
local function ExplodeAtMark(param1, mcoord)
 ...
 if GROUP:FindByName(param1)
  ...
  then local ExpGroup = GROUP:FindByName(param1)
       local ExpUnits = ExpGroup:GetUnits()
    
       for i, Victim in pairs(ExpUnits)
       
        do Victim:Explode(100, 1)
        
       end
 
 elseif tonumber(param1)
 
  then mcoord:Explosion(param1, 1)
  
 else mcoord:Explosion(100, 1)
       
 end      

 if DebugMode == true
  
    then trigger.action.outText("DEBUG: Explosion gezuendet!", 10)
...
local function CargoPickupMov(param1, param2, mcoord)
  ...
  if GROUP:FindByName(param2)
    ...
    then local CarrierGrp = GROUP:FindByName(param2)
         local BoardGroup = CARGO:FindByName(param1)
  
         BoardGroup:Board(CarrierGrp)
...
local function CargoUnboardMov(param1, mcoord)
  ...
  if CARGO:FindByName(param1)
    ...
    then local UnBoardGroup = CARGO:FindByName(param1)
         
         UnBoardGroup:UnBoard(mcoord)
                ...
                then local full = nil
                     ...
                     elseif string.find(cmd, "board")
                      ...
                      then if DebugMode == true
                           ...
                           CargoPickupMov(param1, param2, mcoord)
